...
class WaUserManager(FilebasedUserManager,
    ...
    def findUser(self, userid=None, session=None):
        ...
        if userid and not local_user:
            ...
            if local_user:
                self._logger.debug("Returning local user" + str(local_user));
            return local_user
    ...
    def checkPassword(self, username, password):
        ...
        if user and FilebasedUserManager.checkPassword(self, username, password):
            self._logger.debug("Authed local user " + username)
            return True;
        try:
            ...
            self._logger.debug("Authenticated WA user " + username + "(" + waUser.FirstName + " " + waUser.LastName + ")");
        except:
            self._logger.info("Error authenticating " + username)
            return False

        if waUser.Email != username:
            self._logger.info("Error: WA Username != username");
            return False
